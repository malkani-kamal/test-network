...
export COMPOSE_PROJECT_NAME=spydra
...
function networkDown() {
  ...
  #km COMPOSE_APP_FILES="-f compose/${COMPOSE_FILE_APP}"
  COMPOSE_FILES="${COMPOSE_BASE_FILES} ${COMPOSE_COUCH_FILES} ${COMPOSE_CA_FILES} ${COMPOSE_APP_FILES}"
...
function applicationUp {
  ...
  COMPOSE_FILES="${COMPOSE_FILES} -f compose/${COMPOSE_FILE_COUCH} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_COUCH}"
  COMPOSE_FILES="${COMPOSE_FILES} -f compose/$COMPOSE_FILE_CA"
  #km COMPOSE_FILES="${COMPOSE_FILES} -f compose/$COMPOSE_FILE_APP"

  DOCKER_SOCK="${DOCKER_SOCK}" ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} up -d 2>&1

  $CONTAINER_CLI ps -a
  if [ $? -ne 0 ]; then
    fatalln "Unable to start app"
  fi
}
...
CLI_DELAY=3
# channel name defaults to "spydrachannel"
CHANNEL_NAME="spydrachannel"
# chaincode name defaults to "NA"
# CC_NAME="asset"
CC_NAME="erc20"
# chaincode path defaults to "NA"
# CC_SRC_PATH="../asset-contract"
# CC_SRC_PATH="./chaincodes/go/assets"
CC_SRC_PATH="./chaincodes/go/erc20HTLC"
...
# Compose file to start the app
COMPOSE_FILE_APP=compose-app.yaml
...
if [ "$MODE" == "up" ]; then
  ...
  deployCCAAS
elif [ "$MODE" == "applicationUp" ]; then
  infoln "Starting spydra appications"
  applicationUp
else
  printHelp
  exit 1
